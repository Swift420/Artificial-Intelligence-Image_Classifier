...
# ╔═╡ c4751600-d5b4-11ec-34b2-913f5b69b1b0
using CSV, DataFrames, RDatasets, StatsBase, Statistics, PlutoUI, Flux,Plots ,Images, ImageIO, MLUtils,IterTools

# ╔═╡ 300d1e2c-4612-4c7d-9977-8e1842b50054
using Flux: onehotbatch

# ╔═╡ 8040a385-62ee-4947-bc63-8cc0e7ade459
using Flux: flatten

# ╔═╡ 90385a6a-4e1e-4857-9861-0de6e05cafd2
function resize(directory, im_name, width::Int64, height::Int64)
    resized = Gray.(load(directory * "/" * im_name)) |> (x -> (imresize(x, width, height))) 
    ...
    try
		...
		save("preprocessed_" * directory * "/" * im_name, resized)
    catch e
...
# ╔═╡ 5c471677-9a44-4f44-83de-3afe76a0c63f
function process_images(directory, width::Int64, height::Int64)
    files_list = readdir(directory)
    map(x -> resize(directory, x, width, height),                               files_list)
end


# ╔═╡ bab3a2ef-3df9-4f94-87e9-94b4f98d684d
n_resolution = 224

# ╔═╡ 6bdd99e5-8559-4672-94b8-968fb4ff00ba
begin
    process_images("chest_xray/train/NORMAL/", n_resolution, n_resolution)
    process_images("chest_xray/train/PNEUMONIA/", n_resolution, n_resolution)

end


# ╔═╡ 4eb14ee1-2af6-4247-a6c3-8c6d5bc5addb
begin
    NORMAL_dir = readdir("preprocessed_chest_xray/train/NORMAL/")
    PNEUMONIA_dir = readdir("preprocessed_chest_xray/train/PNEUMONIA/")

end


# ╔═╡ 6d69de01-590d-4656-bd0d-e41eb900c6df
begin
    # we load the pre-proccessed images
    Normal = load.("preprocessed_chest_xray/train/NORMAL/" .* NORMAL_dir)
    PNEUMONIA = load.("preprocessed_chest_xray/train/PNEUMONIA/" .* PNEUMONIA_dir)
   
end


# ╔═╡ 6010d286-b7ff-4864-ac5d-e414d7b72aa8
size(Normal[1])

# ╔═╡ f7cd58ad-e36a-488c-9c79-fc7a06c6368b
data = vcat(Normal, PNEUMONIA);

# ╔═╡ 4ac69f4c-c22d-4e90-8f1f-6738f7c4d880
begin
...
# ╔═╡ 5705aabf-4314-48be-8263-143283431fa8
...
# ╔═╡ 51000c29-541c-4bc3-8676-851b7db1777a
begin
...
# ╔═╡ fdf78af7-2f5e-40fb-a46c-c193adc66791
model = Chain(
        Conv((11, 11), 1=>92, stride=(4,4), relu), #Conv 1
        MaxPool((3,3), stride=(2,2)), # MaxPool 1
		
        Conv((5, 5), 92=>256, pad=2,stride=(2,2), relu),  #Conv 2
        MaxPool((3,3),stride=(2,2) ), #MaxPool 2

        Conv((3, 3), 256=>384, pad=1,stride = (1,1), relu), #Conv 3

		Conv((3, 3), 384=>384, pad=1,stride=(1,1), relu), #Conv 4

		Conv((3, 3), 384=>256, pad=1,stride=(1,1), relu), #Conv 5
		MaxPool((3,3),stride=(2,2) ), #MaxPool 3
        flatten,
		Dense(1024,4096,  relu), #Fully Connected Layer 1
		Dropout(0.5),

	Dense(4096,4096), #Fully Connected Layer 2
	Dropout(0.5),
...
# ╔═╡ 8787c8b6-a637-41e3-8d2d-0399007b0d2d
begin
    train_loss = Float64[]
    test_loss = Float64[]
    acc = Float64[]
    ...
    function update_loss!()
        push!(train_loss, mean(loss.(train_set)))
        push!(test_loss, mean(loss(test_set)))
        push!(acc, accuracy(test_set..., model))
		rounded_train_loss = round(train_loss[end], digits=2)
		rounded_test_loss =  round(test_loss[end], digits=2)
		rounded_acc = round(acc[end], digits=2)
        println("train loss = $rounded_train_loss, test loss = $rounded_test_loss, accuracy = $rounded_acc\n")
    end
end


# ╔═╡ 106aac82-ccfc-4a1b-b14e-5a7ffe83a521
for epoch in 1:2
...
# ╔═╡ 6ba8dcbd-d010-436c-8498-22bc5393e250
begin
